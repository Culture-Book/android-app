    ...
    onPasswordChange: (String, String) -> Unit) 
{
    ...
    ModalBottomSheet(onDismiss = onDismiss,
        onConfirm = {
            showError = oldPassword.isEmpty() || newPassword.isNotValidPassword() || confirmPassword.isNotValidPassword()
            if (!showError) {
                onPasswordChange(oldPassword, newPassword)
                onDismiss()
            }
        }) {
        PasswordField(value = oldPassword, confirmPassword = oldPassword, onValueChanged = { oldPassword = it })
        PasswordField(value = newPassword, confirmPassword = confirmPassword, onValueChanged = { newPassword = it })
        ConfirmPasswordField(value = confirmPassword, password = newPassword, onValueChanged = { confirmPassword = it })
    }
}